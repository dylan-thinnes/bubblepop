...
{-# LANGUAGE BangPatterns, CPP, DefaultSignatures, DeriveDataTypeable, DeriveGeneric, FlexibleInstances, RankNTypes, RoleAnnotations, ScopedTypeVariables #-}
-- | contains a prettyprinter for the
-- Template Haskell datatypes

module Bubble.GHC.Ppr where
    -- All of the exports from this module should
    -- be "public" functions.  The main module TH
    -- re-exports them all.

import Text.PrettyPrint.Annotated (render, renderSpans, Span, annotate)
import Bubble.GHC.PprLib
...
import "ghc-boot-th" GHC.Lexeme( startsVarSym )
...
pprintWithSpans :: Ppr a => a -> (String, [Span Ann])
pprintWithSpans x = renderSpans $ to_HPJ_Doc $ ppr x
...
------------------------------
instance Ppr (ExpG Redex) where
    ppr = pprExpG noPrec

pprInfixExpG :: ExpG Redex -> Doc
pprInfixExpG (FE _ _ (VarEF v))     = pprName' Infix v
pprInfixExpG (FE _ _ (ConEF v))     = pprName' Infix v
pprInfixExpG (FE _ _ _)             = text "<<Non-variable/constructor in infix context>>"
pprInfixExpG (FP _ _ (PrimOp {..})) = pprName' Infix primOpName

pprMaybeExpG :: Precedence -> Maybe (ExpG Redex) -> Doc
pprMaybeExpG _ Nothing = empty
pprMaybeExpG i (Just e) = pprExpG i e

pprExpG prec (FE i redex exp) = ann <$> str
  where
      ann = case redex of
              NoRedex -> id
              Redex _ -> annotate i
      str = pprExpG' prec exp

pprExpG' :: Precedence -> ExpF (ExpG Redex)  -> Doc
pprExpG' _ (VarEF v)     = pprName' Applied v
pprExpG' _ (ConEF c)     = pprName' Applied c
pprExpG' i (LitEF l)     = pprLit i l
pprExpG' i (AppEF e1 e2) = parensIf (i >= appPrec) $ pprExpG opPrec e1
                                              <+> pprExpG appPrec e2
pprExpG' i (AppTypeEF e t)
 = parensIf (i >= appPrec) $ pprExpG opPrec e <+> char '@' <> pprParendType t
pprExpG' _ (ParensEF e)  = parens (pprExpG noPrec e)
pprExpG' i (UInfixEF e1 op e2)
 = parensIf (i > unopPrec) $ pprExpG unopPrec e1
                         <+> pprInfixExpG op
                         <+> pprExpG unopPrec e2
pprExpG' i (InfixEF (Just e1) op (Just e2))
 = parensIf (i >= opPrec) $ pprExpG opPrec e1
                        <+> pprInfixExpG op
                        <+> pprExpG opPrec e2
pprExpG' _ (InfixEF me1 op me2) = parens $ pprMaybeExpG noPrec me1
                                    <+> pprInfixExpG op
                                    <+> pprMaybeExpG noPrec me2
pprExpG' i (LamEF [] e) = pprExpG i e -- #13856
pprExpG' i (LamEF ps e) = parensIf (i > noPrec) $ char '\\' <> hsep (map (pprPat appPrec) ps)
                                           <+> text "->" <+> ppr e
pprExpG' i (LamCaseEF ms) = parensIf (i > noPrec)
                       $ text "\\case" $$ nest nestDepth (ppr ms)
pprExpG' _ (TupEF es) = parens (commaSep es)
pprExpG' _ (UnboxedTupEF es) = hashParens (commaSep es)
pprExpG' _ (UnboxedSumEF e alt arity) = unboxedSumBars (ppr e) alt arity
-- Nesting in Cond is to avoid potential problems in do statements
pprExpG' i (CondEF guard true false)
 = parensIf (i > noPrec) $ sep [text "if"   <+> ppr guard,
                       nest 1 $ text "then" <+> ppr true,
                       nest 1 $ text "else" <+> ppr false]
pprExpG' i (MultiIfEF alts)
  = parensIf (i > noPrec) $ vcat $
      case alts of
        []            -> [text "if {}"]
        (alt : alts') -> text "if" <+> pprGuarded arrow alt
                         : map (nest 3 . pprGuarded arrow) alts'
pprExpG' i (LetEF ds_ e) = parensIf (i > noPrec) $ text "let" <+> pprDecs ds_
                                             $$ text " in" <+> ppr e
  where
    pprDecs []  = empty
    pprDecs [d] = ppr d
...
pprExpG' i (CaseEF e ms)
 = parensIf (i > noPrec) $ text "case" <+> ppr e <+> text "of"
                        $$ nest nestDepth (ppr ms)
pprExpG' i (DoEF ss_) = parensIf (i > noPrec) $ text "do" <+> pprStms ss_
  where
    pprStms []  = empty
    pprStms [s] = ppr s
    pprStms ss  = braces (semiSep ss)
pprExpG' i (MDoEF ss_) = parensIf (i > noPrec) $ text "mdo" <+> pprStms ss_
  where
    pprStms []  = empty
    pprStms [s] = ppr s
    pprStms ss  = braces (semiSep ss)

pprExpG' _ (CompEF []) = text "<<Empty CompExpG>>"
...
pprExpG' _ (CompEF ss) =
  ...
  where s = last ss
        ss' = init ss
pprExpG' _ (ArithSeqEF d) = ppr d
pprExpG' _ (ListEF es) = brackets (commaSep es)
pprExpG' i (SigEF e t) = parensIf (i > noPrec) $ pprExpG sigPrec e
                                          <+> dcolon <+> ppr t
pprExpG' _ (RecConEF nm fs) = ppr nm <> braces (pprFields fs)
pprExpG' _ (RecUpdEF e fs) = pprExpG appPrec e <> braces (pprFields fs)
pprExpG' i (StaticEF e) = parensIf (i >= appPrec) $
                         text "static"<+> pprExpG appPrec e
pprExpG' _ (UnboundVarEF v) = pprName' Applied v
pprExpG' _ (LabelEF s) = text "#" <> text s
pprExpG' _ (ImplicitParamVarEF n) = text ('?' : n)
...
------------------------------
pprGuarded :: Ppr a => Doc -> (Guard, a) -> Doc
